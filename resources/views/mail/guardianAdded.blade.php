<!doctype html>
<html lang="en">
  <head>
    <title>NDIS</title>
    @include('mail.commonCss')
  </head>
  <body>
    <div class="header-email"><h4>NDIS</h4></div>
    <div class="table-email">
        <div class="table-custom-email">
            <h3 style="">{{ sprintf('Hi %s,', $notifiable->getName() )}}</h3>
            <p>A Guardian for your participant account with {{ config('app.name') }} has been added successfully!</p>
            <p>----- Details -----</p>
            <p>Name: {{ $guardian->name }}</p>
            <p>Email: {{ $guardian->email }}</p>
            <p>Address: {{ $guardian->address }}</p>
            <p>Phone: {{ $guardian->phone }}</p>
            <p>Mobile: {{ $guardian->mobile }}</p>
            <p>If you do not have requested for any guardian or you want to update any of the above details, pleae contact NDISCentral.</p>
            <p style="margin-top: 20px;">Thank you for using {{config('app.name')}}!</p>
        </div>
    </div>
  <div class="footer-email"><p><span>&#169;  </span><span>{{ now()->year }} NDIS All rights reserved</span></p></div>
  </body>
</html>