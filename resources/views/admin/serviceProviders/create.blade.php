...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('serviceProvider.title_singular') }}
    </div>

    <div class="card-body">
        <form action="{{ route("admin.provider.store") }}" method="POST" class="validated" enctype="multipart/form-data">
            @csrf
            <div class="row">
                {!! 
                ...
                {!! 
                    Form::location('address', trans('serviceProvider.fields.address'), old('address', isset($serviceProvider) ? $serviceProvider->address : '') )
                    ->id('address')
                    ->attrs([
                        "required"=>"required", 
                        "data-rule-required"=>"true", 
