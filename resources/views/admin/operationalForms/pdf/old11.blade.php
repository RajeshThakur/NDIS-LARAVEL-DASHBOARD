            ...
            <div class="below-form">
               ...
               <div class="Summary-section serviceagreement-additional">
                  <div class="row">
                     <label>
                        ...
                        {!! trans('opforms.fields.agreement_commence_on') !!}
                        
                        {!! trans('opforms.fields.for_period') !!}   
                        
                     </label>
                     {!! 
                        Form::date('meta[aggrement_start_date]', trans('opforms.fields.date'), old('aggrement_start_date', isset($meta['aggrement_start_date']) ? $meta['aggrement_start_date'] : date('d-m-Y')))
                        ->id('date-1')
                        ->size('col-sm-5')
                        ->required('required')
                        ->attrs([ "required"=>"required", "data-rule-required"=>"true", "data-msg-required"=>"Agreement start date required" ])
                        ->help(trans('global.user.fields.name_helper'))
                     !!}
                     
                     <div class="col-sm-2 text-center pt-3">To</div>

                     {!! 
                        Form::date('meta[aggrement_end_date]', trans('opforms.fields.date'), old('aggrement_end_date', isset($meta['aggrement_end_date']) ? $meta['aggrement_end_date'] : date('d-m-Y')))
                        ->id('date-2')
                        ->size('col-sm-5')
                        ->required('required')
                        ->attrs([ "required"=>"required", "data-rule-required"=>"true", "data-msg-required"=>"Agreement end date required" ])
                        ->help(trans('global.user.fields.name_helper'))
                     !!}                        
               ...
               <div class="payment-section">
                  ...
                  <div class="form-group {{ $errors->has('registeration_group') ? 'has-error' : '' }}">
                        ...
                        <select name="registeration_group[]" id="registeration_group" class="form-control select2" required="required" data-rule="required" data-msg-required="Registration group selection required" multiple="multiple"> 
               ...
               <div class="agreeement-section">
                  ...
                  {!! 
                     ...
                     ->attrs([ "required"=>"required", "data-rule-required"=>"true", "data-msg-required"=>"Mobile no required" ])
                  ...
                  {!! 
                     Form::location('meta[participant_address]', 
                                       trans('opforms.fields.address'), 
                                       issetKey( $meta, 'participant_address', '' ),
                                       issetKey( $meta, 'participant_lat', '' ), 
                                       issetKey( $meta, 'participant_lng', '' ) 
                                    )
               ...
               <div class="agreeement-section">
                  ...
                  {!! 
                     Form::location('meta[provider_address]', 
                                       trans('opforms.fields.address'), 
                                       issetKey( $meta, 'provider_address', '' ),
                                       issetKey( $meta, 'provider_lat', '' ), 
                                       issetKey( $meta, 'provider_lng', '' ) 
                                    )
                     ->id('provider_address')
                     ->locationLatName('meta[provider_lat]')
                     ->locationLngName('meta[provider_lng]')
                     ->required('required')
                     ->attrs([ "required"=>"required", "data-rule-required"=>"true", "data-msg-required"=>"Provider address required" ])
                     ->help(trans('global.user.fields.name_helper'))
                  !!}
               ...
               <div class="agreeement-section">
                  ...
                  {!! 
                     Form::date('meta[signature_date]', trans('opforms.fields.date'), old('signature_date', isset($meta['signature_date']) ? $meta['signature_date'] : date('d-m-Y')))
                     ->id('date-3')
                     ->required('required')
                     ->attrs([ "required"=>"required", "data-rule-required"=>"true", "data-msg-required"=>"Provider signature date required" ])
                     ->help(trans('global.user.fields.name_helper'))
                  !!}
                  ...
                  {!! 
                     Form::date('meta[signature_date_auth]', trans('opforms.fields.date'), old('signature_date_auth', isset($meta['signature_date_auth']) ? $meta['signature_date_auth'] : date('d-m-Y')))
                     ->id('date-4')
                     ->required('required')
                     ->attrs([ "required"=>"required", "data-rule-required"=>"true", "data-msg-required"=>"Authorised signature date required" ])
                     ->help(trans('global.user.fields.name_helper'))
                  !!}
               ...
               <div class="agreeement-section">
                     ...
                     {!! 
                        Form::date('meta[policy_review_date]', 'Review date:', old('policy_review_date', isset($meta['policy_review_date']) ? $meta['policy_review_date'] : date('d-m-Y')))
                        ->id('date-5')
                        ->required('required')
                        ->attrs([ "required"=>"required", "data-rule-required"=>"true", "data-msg-required"=>"Review date required" ])
                        ->help(trans('global.user.fields.name_helper'))
                     !!}
                     {!! 
                        Form::date('meta[original_review_date]', 'Original issue:', old('original_review_date', isset($meta['original_review_date']) ? $meta['original_review_date'] : date('d-m-Y')))
                        ->id('date-6')
                        ->required('required')
                        ->attrs([ "required"=>"required", "data-rule-required"=>"true", "data-msg-required"=>"Original review date required" ])
                        ->help(trans('global.user.fields.name_helper'))
                     !!}
                     {!! 
                        Form::date('meta[current_version_date]', 'Current version:', old('current_version_date', isset($meta['current_version_date']) ? $meta['current_version_date'] : date('d-m-Y')))
                        ->id('date-7')
                        ->required('required')
                        ->attrs([ "required"=>"required", "data-rule-required"=>"true", "data-msg-required"=>"Current version date required" ])
                        ->help(trans('global.user.fields.name_helper'))
                     !!}
