<form action="{{ route($formAction[0],$formAction[1]) }}" method="POST" class="validated" enctype="multipart/form-data">
   ...
   @method('PUT')        
   @endif
   <input type="hidden" name="template_id" value="13">
   <input type="hidden" name="user_id" value='{{$participant->user_id}}'>
   <input type="hidden" name="worker_controller" value='{{$participantController}}'>
   <input type="hidden" name="agreement_with" value='{{$agreementName}}'>
   <div class="operational-form-design support-agreement">
      <div class="authority-section">
         ...
         <div class="row">
            ...
            <div class="label col-sm-8">
               {!! 
               Form::text('sw_esw_name', trans('opforms.fields.sw_ew'), old('sw_esw_name', $participant->getName() ) )
               ->id('sw_esw_name')
               ->required('required')
         ...
         <div class="row">
            ...
            <div class="label col-sm-8">
               ...
               ->id('provider_name')
               ->required('required')
               ->attrs([ "required"=>"required", "data-rule-required"=>"true", "data-msg-required"=>"Service Provider name required" ])
               ->hideLabel()
               ->help(trans('global.user.fields.name_helper'))
               !!}
            </div>
         ...
         <div class="row">
            ...
            <div class="label col-sm-8">
               <span class="count_registration_groups_selected_options dashb-count"></span>
               <div class="reg-group">
                  <div class="input-group" data-date-format="dd-mm-yyyy">
                     <select name="registeration_group[]" required="required" data-rule="required" data-msg-required="Registration group selection required" id="registration_groups" class="form-control select2" multiple>
                        <option disabled>Select registration group</option>
                        @foreach($registration_groups as $id => $registration_groups)
                        <option value="{{ $id }}" {{ (in_array($id, old('registeration_group', [])) || isset($participant) && $participant->reg_grps->contains( 'reg_group_id', $id )) ? 'selected' : '' }}>{{ $registration_groups }}</option>
                        @endforeach
                     </select>
         ...
         <div class="col-sm-12 mt-40">
            ...
            <div class="row">
               ...
               <div class="label col-sm-8">
                  ...
                  <img alt="worker_signature" src="{{ $meta['worker_signature'] }}" />
                  @else
   ...
   <button class="btn btn-primary rounded" id="button" type="submit">Submit</button>
</form>