...
<div class="card">   
    <div class="card-header">
        <div class="row">
            ...
            <div class="pageTitle">
            <h2>Reports - Service Bookings By Time Period</h2>
            </div>            
        </div>
    </div>
    ...
    <div class="serchbaar">
            <form action="{{ route("admin.reports.service-bookings") }}" method="GET" class="m-0" role="search">
                ...
                <div class="row">                    
                    {!!
                        // Form::select('member', trans('reports.search_name'),$participant,($participant_detail ? $participant_detail : old('member')))
                        // ->id('member')
                        // ->size('col-sm-4')
                        Form::text('member', 'Support Worker / External Services', '')->size('col-sm-4')->id('member')->placeholder('External Services, Support Worker etc.')
                    !!}

                    {{-- <select name="member" >
                        @foreach($allParticipants as $key=>$participant)
                            <option value="{{$key}}">{{ $participant }}</option>
                        @endforeach
                    </select> --}}
