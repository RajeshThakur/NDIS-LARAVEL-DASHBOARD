...
class CreateContentCategoryContentPagePivotTable extends Migration
{
    public function up()
    {
        Schema::create('content_category_content_page', function (Blueprint $table) {
            ...
            $table->foreign('content_page_id', 'content_page_id_fk_170136')->references('id')->on('content_pages');
            $table->unsignedInteger('content_category_id');
            $table->foreign('content_category_id', 'content_category_id_fk_170136')->references('id')->on('content_categories');
        });
    }
}
