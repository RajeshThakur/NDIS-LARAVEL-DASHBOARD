...
class SWProviderScope implements Scope
{
    /**
    ...
    public function apply(Builder $builder, Model $model)
    {
        if(\Auth::user()){
            ...
            if( !$builder->getQuery()->columns && empty($builder->getQuery()->columns) ){
                $builder->addSelect('users.*','support_workers_details.*', 'support_workers_details.id as sw_id');
            }
            $builder->leftJoin('users_to_providers', 'support_workers_details.user_id', '=', 'users_to_providers.user_id');
