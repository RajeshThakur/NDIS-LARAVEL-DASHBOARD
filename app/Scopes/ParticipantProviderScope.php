...
class ParticipantProviderScope implements Scope
{
    /**
    ...
    public function apply(Builder $builder, Model $model)
    {
        if(\Auth::user()){
            ...
            if( !$builder->getQuery()->columns && empty($builder->getQuery()->columns) ){
                $builder->select( 'users.*', 'participants_details.id as participant_id', 'participants_details.*' );
            }
            
            $builder->leftJoin('users_to_providers', 'participants_details.user_id', '=', 'users_to_providers.user_id');
