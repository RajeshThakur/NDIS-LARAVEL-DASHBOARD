...
class Documents extends Model
{
    ...
    public static function getParticipantDocuments( $participantId ){
        ...
        $provider = \Auth::user();
        return Documents::where('users_to_providers.provider_id', $provider->id)->where('participants_details.user_id', $participantId)
    ...
    public static function getUserDocuments( $user_id ){
      ...
      return Documents::where('documents.provider_id', $provider->id)->where('documents.user_id', $user_id)
    ...
    public static function saveDoc( \Illuminate\Http\UploadedFile $file, $docAttr = [] ){
        ...
        try{
          ...
          $document->s3_bucket = env('AWS_BUCKET');
    ...
    public static function saveFileGetInfo( \Illuminate\Http\UploadedFile $file, $user_id ){
      ...
      try{
        ...
        $fileInfo->fileURL = 'https://'.env('AWS_BUCKET').'.s3.' . env('AWS_DEFAULT_REGION') . '.amazonaws.com/'.$fileInfo->filePath;
